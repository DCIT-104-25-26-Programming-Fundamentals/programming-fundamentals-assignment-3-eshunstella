 ...
 *
 * @param {number[]} scores - The student's scores.
 * @returns {number} The average score (0 if the array is empty).
 */
...
/**
 * Prompts the user for a student's name, ID, and scores, then saves
 * the new student object to the students array.
 */
function addStudent() {
...
function displayAllStudents() {
  ...
  for (let i = 0; i < students.length; i++) {
    ...
    const avgText = average.toFixed(2);

    console.log(`${name} ${id} ${scores} ${avgText}`);
  }
}

/**
 * Asks the user for a student ID, finds the matching student, and
 * prints their average score. Prints an error if the ID isn't found.
 */
 ...
 * to the matching feature until the user chooses to quit.
 */
function main() {
  let running = true;
